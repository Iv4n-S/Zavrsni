...
function AdminHotelSearch(props) {
    ...
    function onSubmit() {
        if(locationSearch == "") {
            ...
            fetch(NETWORK_CONFIG.apiFullHost + API_CONFIG.searchAdminHotels, options)
                .then((response) => {
                    if(!response.ok) {
                        ...
                        response.json().then((value) => {
                            if(value.length == 0) {
                                setError(`There are no hotels in ${locationSearch}`);
                            }
                            else {
    ...
    return (
        <Card className="md:w-2/3 w-auto">
            ...
            <div className="flex justify-center pt-2 my-2">
                <Button
                    type="button"
